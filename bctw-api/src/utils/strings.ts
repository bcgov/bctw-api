...
const ErrorMsgs = {
  ...
  metadata: {
    ...
    missingData:
      'This row is missing a required field. Ensure you have entered a device ID, a device make, sex, and a capture date.',
    badMarkings:
      'This animal has insufficient marking information to uniquely identify it.',
  ...
  fields: {
    ...
    number: {
      ...
      help: 'This field is set to only accept numbers, including integers and floating points. Ensure you have not included any special characters.',
    },
    bool: {
...
const importMessages = {
  warningMessages: {
    previousDeployment: {
      message: (device_id: string | number): string =>
        `I am re-using Device ID ${device_id} for a new deployment.`,
      help:
        'We found records of other deployments for this device ID in our database, but none of them overlapped the time period you are trying to import.' +
        " You can safely import this deployment if you are sure it differs from what's already been tracked.",
    },
    manyDeviceOneAnimal: {
      message:
        'I am attaching multiple devices to this animal over the same time span.',
      help:
        'Our system allows animals to wear multiple devices over the same time span.' +
    ...
    matchingMarkings: {
      message:
        'I am importing a new animal, even though this same combination of markings identifies an existing animal.',
      help: (
        species: string
      ): string => `We already have records of a ${species} that matches the markings you have provided.
