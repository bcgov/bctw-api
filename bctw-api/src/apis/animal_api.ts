...
const _getAttachedSQL = (
  ...
  const base = `
    WITH ${alias} AS (
      SELECT
        c.assignment_id, c.device_id, c.collar_id, c.frequency, c.device_status,
        c.attachment_start, c.data_life_start, c.data_life_end, c.attachment_end, latitude, longitude,
        ${
          getAllProps
            ? 'a.*,'
