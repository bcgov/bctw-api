...
module.exports = (on, config) => {
  ...
  const mergeAndInsert = async (
    ...
    console.log(
      `last successfull insertion to ATS table was ${lastEntry.format()}`
    );

    // filter out old data
    const newCollarEvents = filterCollarDataAfter(data, lastEntry);
    const newTransmissions = filterTransmissionDataAfter(
      transmissionData,
      lastEntry
    );
    console.log(
      `filtered results: data entries: ${newCollarEvents.length}, transmission entries: ${newTransmissions.length}`
    );

    // combine entries from both the files into a single bctw.telemetry_api_ats record
  ...
  async function handleParseAndInsert() {
    ...
    if (paths.length !== 2) {
      console.log(
        `cypress downloading tests completed. ${downloadPath} contains ${
          paths.length ? paths.join() : "no files"
        }. exiting script early.`
      );
      return null;
    }
    console.log(
      `collar event data at ${paths[0]}\ntransmission data at ${paths[1]}`
    );
    ...
    console.log(
      `completed parsing files downloaded files to JSON, ${collarData.length} collar event data and ${transmissionData.length} transmission events`
    );
