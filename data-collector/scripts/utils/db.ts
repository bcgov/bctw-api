...
const pool = {
  ...
  port,
  max: 10,
  idleTimeoutMillis: TIMEOUT_MILLIS,
};
 ...
 */
const getLastAlertTimestamp = async (
  alert_table: string,
  device_make: eVendorType
): Promise<Dayjs | null> => {
 ...
 */
const getIsDuplicateAlert = async (
  alert_table: string,
  device_id: number,
  device_make: eVendorType,
  alertType: string
...
const getLastKnownLatLong = async (
  device_id: number,
  device_make: string,
  alert_time: string
): Promise<latLong> => {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (!idleCount && !waitingCount) {
        ...
        console.log(
          `Connections still active... IdleCount: ${pgPool.idleCount} | WaitingCount: ${pgPool.waitingCount}`
        );
        resolve(false);
      }
    }, 3000);
...
const safelyDrainAndClosePool = async (retries: number) => {
  while (true) {
    const isEmpty = await isPoolEmpty();
    if (isEmpty) {
      ...
      await pgPool
        ...
        .catch((err) =>
          console.log(`Problem occured when closing pool connection...`, err)
        );
      break;
    }
    if (!retries) {
      console.log(
        `DB tried ${retries} times to close connection and was not successful. Aborting...`
      );
      break;
    }
    retries--;
...
const transactionify = (sql: string) =>
  isProd ? sql : `begin; ${sql}; rollback;`;
